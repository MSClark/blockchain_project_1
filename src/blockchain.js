...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            //get current time
            let currentTime = self._getCurrentTimeStamp()
            //check time elapsed
            const isValidEpoch = currentTime - messageTime < FIVE_MIN
            //verify message
            const isValid = bitcoinMessage.verify(message, address, signature)
            //create and add block
            let data = {address, message, signature, star}
            isValidEpoch && isValid ? resolve(self._addBlock(new BlockClass.Block(data))) :
                reject(`Error - Valid time window: ${isValidEpoch}, Is valid: ${isValid}`)
        });
    }
