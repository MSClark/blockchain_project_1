...
class BlockchainController {
    ...
    getChainHeight() {
        this.app.get("/block/height", async (req, res) => {
            return res.status(200).json(this.blockchain.height);
        });
    }
    ...
    getChainValidation() {
        this.app.get("/chain/validate", async (req, res) => {
            let data = await this.blockchain.validateChain()
                .then(data => {
                    console.log("validate chain controller data " + data)
                }).catch(error => {
                    console.log(error)
                })
            //data.isArray() ? return res.status(500).json(data) : return res.status(200).json("Valid")
